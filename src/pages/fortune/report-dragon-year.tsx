...
function ReportDetail() {
        ...
        for (;;) {
          ...
          if (value) {
            ...
            if (text.startsWith('<start:')) {
              ...
              setReportId(id);
              setUrl(joinURL(window.location.origin, prefix, 'fortune/share/1', id));
            } else {
      ...
      if (reportId) {
        try {
          ...
          if (report.code === 1) {
            ...
            navigate('/fortune/form/1', { replace: true });
          } else if (report.code === 0 && report.report) {
            // 显示数据
            setContent(report.report);
            setUrl(joinURL(window.location.origin, prefix, 'fortune/share/1', reportId));
            setIsGenerating(false);
          }
        } catch (error) {
          console.error(error);
  ...
  return (
    <DivOuter bgUrl={joinURL(window?.blocklet?.prefix ?? '/', '/images/dragon-bg.png')}>
      <Container needScroll={!isOK}>
        <Div>
          <p className="title">{t('fortune.genReport.dragonTitle')}</p>
          ...
          <Collapse in={!isGenerating && !isInAistroClient()} timeout={1000}>
            <div className="bottom-container">
              <div className="bottom" dangerouslySetInnerHTML={{ __html: t('fortune.genReport.bottomTipForDragon') }} />
