      ...
      if (reportId) {
        try {
          ...
          if (report.code === 1) {
            ...
            setUrl(joinURL(window.location.origin, prefix, 'fortune/share/2', reportId));
            setIsGenerating(false);
          }
        } catch (error) {
          console.error(error);
  ...
  return (
    <DivOuter bgUrl={joinURL(window?.blocklet?.prefix ?? '/', '/images/snake-bg.png')}>
      <Container needScroll={!isOK}>
        <Div>
          <p className="title">{t('fortune.genReport.snakeTitle')}</p>
          ...
          <Collapse in={!isGenerating && !isInAistroClient()} timeout={1000}>
            <div className="bottom-container">
              <div className="bottom" dangerouslySetInnerHTML={{ __html: t('fortune.genReport.bottomTipForSnake') }} />
