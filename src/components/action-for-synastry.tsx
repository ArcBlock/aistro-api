...
import { InviteFriend } from '../libs/type';
...
export function OpenAistroButton({ inviteId }: { inviteId: string | undefined }) {
  const { t } = useLocaleContext();
  return (
    <ButtonInfo href={`aistro://aistro.io/f?inviteId=${inviteId || ''}`}>
...
function ActionForSynastry({
  onLoggedIn,
  inviteFriend,
}: {
  onLoggedIn: () => void;
  inviteFriend: InviteFriend | null | undefined;
}) {
    ...
    return (
      <Container>
        <Stack spacing={1} direction="column">
          ...
          <AppleButton />
          <OpenAistroButton inviteId={inviteFriend?.id} />
        </Stack>
      </Container>
    );
  }
...
export default ActionForSynastry;
